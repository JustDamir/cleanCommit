...
const translations = {
    ru: {
        language: "[язык]",
        code_input: "[ввод_кода]",
        auto_detect: "Автоопределение",
        auto_hint: "# автоопределение языка (включено)",
        check_button: "Проверить код",
        waiting_text: "[ожидание ввода]",
        waiting_hint: "> введите минимум 50 символов для анализа",
        min_chars: "$ min: 50 символов",
        code_placeholder: "> вставь код сюда...\n\n  def analyze_soul(code):\n      return \"позволь мне почувствовать его душу\"",
        detected_prefix: "$ определен:",
        selected_prefix: "$ выбран:",
        not_detected: "не определен",
        language_detected: "# определен язык:",
        language_selected: "# выбран язык:",
        not_detected_hint: "# язык не определен (попробуйте выбрать вручную)",
        enter_code_hint: "# введите код для автоопределения языка",
        characters: "$ символов:",
        lines: "$ строк:"
    },
    en: {
        ...
        auto_hint: "# auto-detection enabled",
        check_button: "Check code",
        waiting_text: "[waiting for input]",
        waiting_hint: "> enter at least 50 characters for analysis",
        min_chars: "$ min: 50 chars",
        code_placeholder: "> paste code here...\n\n  def analyze_soul(code):\n      return \"let me feel its soul\"",
        detected_prefix: "$ detected:",
        selected_prefix: "$ selected:",
        not_detected: "not detected",
        language_detected: "# language detected:",
        language_selected: "# language selected:",
        not_detected_hint: "# language not detected (try manual selection)",
        enter_code_hint: "# enter code for auto-detection",
...
function updateUILanguage() {
    ...
    if (textarea && t.code_placeholder) {
        textarea.placeholder = t.code_placeholder;
    }
...
function updateDetectedLanguageDisplay() {
    ...
    if (isAutoMode && code.trim().length > 20) {
        ...
        if (detection.detected) {
            ...
            detectedLangDisplay.innerHTML = `${t.detected_prefix} ${langName}`;
            detectedLangDisplay.style.color = '#5EB2DB';
            
            if (detectedLanguageHint) {
                detectedLanguageHint.innerHTML = `${t.language_detected} ${langName}`;
                detectedLanguageHint.style.color = '#5EB2DB';
                detectedLanguageHint.style.background = 'rgba(94, 178, 219, 0.1)';
            }
        } else {
            detectedLangDisplay.innerHTML = `${t.detected_prefix} ${t.not_detected}`;
            detectedLangDisplay.style.color = '#AE65DB';
            
            if (detectedLanguageHint) {
                detectedLanguageHint.innerHTML = t.not_detected_hint;
        ...
        detectedLangDisplay.innerHTML = `${t.selected_prefix} ${langName}`;
        detectedLangDisplay.style.color = '#5E8ADB';
        
        if (detectedLanguageHint) {
            detectedLanguageHint.innerHTML = `${t.language_selected} ${langName}`;
            detectedLanguageHint.style.color = '#5E8ADB';
            detectedLanguageHint.style.background = 'rgba(94, 138, 219, 0.1)';
        }
    } else {
        detectedLangDisplay.innerHTML = `${t.detected_prefix} —`;
        detectedLangDisplay.style.color = '#5a6e8a';
        
        if (detectedLanguageHint) {
            detectedLanguageHint.innerHTML = t.enter_code_hint;
...
function updateLineNumbers() {
    ...
    lineCountSpan.textContent = `${t.lines} ${lineCount}`;
}
...
function updateButtonState() {
    ...
    if (isValid) {
        ...
        charCountSpan.innerHTML = `${t.characters} ${nonEmptyChars} ✓`;
    } else {
        submitBtn.disabled = true;
        submitBtn.classList.remove('active');
        charCountSpan.style.color = '#AE65DB';
        const needMore = MIN_CHARS - nonEmptyChars;
        charCountSpan.innerHTML = `${t.characters} ${nonEmptyChars}/${MIN_CHARS} (need ${needMore} more)`;
    }
}
